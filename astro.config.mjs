...
// Phase 2: `/p/[slug]` palette pages are SSR-only, so `@astrojs/sitemap` can't
// enumerate them. We inject a best-effort static seed list (see the module
// header for the D1-regeneration hook - the build process has no DB binding, so
// these can't be queried live here).
import { SITEMAP_PALETTE_SLUGS } from './src/lib/data/sitemap-seed.ts';
