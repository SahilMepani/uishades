...
- **Tier 2 — semantic aliases**, emitted only when `ColorGroup.semantic` is set (the user's editable role label, e.g. Primary). `semanticTokens(group)` applies a fixed default variant set — base (`anchorKey` stop) / `hover` (+1) / `active` (+2) / `surface` 50 / `muted` 100 / `border` 200 / `emphasis` 800 (every variant aliases a stop of the role's own ramp). v4/css-vars emit live `var()` aliases; v3 (a JS config) emits resolved-hex snapshots. The three JSON formats (`w3c-tokens`, `figma-vars`, `style-dictionary`) ignore `semantic` and stay tier-1 only.
