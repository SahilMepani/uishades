 ...
 * end of the ramp, `ceil(ix + (count + 1 - ix) * 0.75)` can reach `count`, which
 * would index one past the last shade and crash the renderer. Clamping to
 * `count - 1` is the fix - matching the `Math.min(ramp.shades.length - 1, …)`
 * pattern the neighbor walk in `api/[hex].json.ts` already uses. The darker-side
 * targets interpolate against `count + 1` (not a literal `21`) so the strip's
 * spread scales with the ramp length.
 */
export function ogStripIndices(ix: number, count: number): number[] {
  ...
  return [
    ...
    ix,
    Math.min(max, Math.ceil(ix + (reach - ix) * 0.35)),
    Math.min(max, Math.ceil(ix + (reach - ix) * 0.75)),
  ];
}
