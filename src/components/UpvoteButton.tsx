...
/**
 * UpvoteButton — the heart/▲ + count control on every PaletteCard.
 *
 * Optimistic toggle: clicking flips `voted`/`count` immediately, then POSTs
 * (vote) or DELETEs (unvote) `/api/palettes/[id]/vote`. The server returns the
 ...
 *
 * Voting REQUIRES sign-in. A signed-out click (or a 401 from the API) opens the
    ...
    <>
      ...
      >
        <svg
          ...
          strokeWidth="1.5"
        >
          <path d="M8 2.5l4.5 5H10v6H6v-6H3.5L8 2.5z" strokeLinejoin="round" />
        </svg>
        <span>{count}</span>
      </button>
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center p-4 pt-[12vh]">
      ...
      >
        ...
        <div className="mb-4 flex flex-col gap-1">
          <span className="kicker">Upvote</span>
          <h2 className="display text-xl">Sign in to vote</h2>
        </div>
